                          ...
                          <div className="text-white font-medium text-sm">
                            ...
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <Badge variant="ghost" className={`px-2 py-1 text-xs font-medium ${
                                (battle.status === 'active' && new Date(battle.ends_at) > new Date()) ? 'bg-green-500/20 text-green-400 border-green-500/20' :
                                (battle.status === 'active' && new Date(battle.ends_at) <= new Date()) || battle.status === 'closed' ? 'bg-gray-500/20 text-gray-400 border-gray-500/20' :
                                battle.status === 'pending' ? 'bg-orange-500/20 text-orange-400 border-orange-500/20' :
                                'bg-blue-500/20 text-blue-400 border-blue-500/20'
                              }`}>
                                {battle.status === 'pending' ? 'Open' : 
                                 (battle.status === 'active' && new Date(battle.ends_at) > new Date()) ? 'Active' :
                                 (battle.status === 'active' && new Date(battle.ends_at) <= new Date()) || battle.status === 'closed' ? 'Ended' : 
                                 battle.status}
                              </Badge>
